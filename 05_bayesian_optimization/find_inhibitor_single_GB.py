...
if __name__ == '__main__':
	...
	save_path = f"single_less_20wt_{model}_26_41"
	...
	if calc:
		print_log(f"./optimization/find_inhibitor_single_less_20wt_{model}_26_41.log","w")
		for ind in indexsss:
			indexss = [ind]
			# -------------------------------------------------------------------------------------------------
			df = pd.read_excel(f)
			...
			for indexs in indexss:
				...
				for pressure in Pressures:
					...
					result_temp = np.array([result.fun,pressure,result.x[0]])
					np.savetxt(f"./optimization/{save_path}/temp/func_vals_{pressure}.temp",func_vals,fmt="%f")
					np.savetxt(f"./optimization/{save_path}/temp/results_{pressure}.temp",result_temp.T,fmt="%f")
				...
				df_func_vals.to_excel(f"./optimization/{save_path}/optimization_func_vals_{select_columns[indexs[0]]}.xlsx", index=False)
				# print(temperature_obs,Pressures, c_inhibitors)
				ob = np.column_stack((temperature_obs,Pressures, c_inhibitors)) #.reshape(1,-1)
				df_results = pd.DataFrame(ob, columns=["Objective T (K)", "Objective P (MPa)", f"{select_columns[indexs[0]]}"])
				# Save predicted temperature, pressure, and inhibitor concentrations
				df_results.to_excel(f"./optimization/{save_path}/optimization_results_{select_columns[indexs[0]]}.xlsx", index=False)
	...
	# df_func_vals = pd.read_excel(f"./optimization/{save_path}/optimization_func_vals_{select_columns[indexss[0][0]]}.xlsx")
	# print(df_func_vals)
	...
	# df_results = pd.read_excel(f"./optimization/{save_path}/optimization_results_{select_columns[indexss[0][0]]}.xlsx")
	# print(df_results)
