...
if __name__ == '__main__':
	...
	save_path = f"three_less_20wt_{model}"
	...
	#0.1~20%  [11,17,30,39] 
	# [13,18,21],[13,18,37],[13,21,37],[18,21,37] # All concentration combinations
	
	[11,17,30],[11,17,39],[11,30,39],[17,30,39] # 0.1~20%
	...
	if calc:
		print_log(f"./optimization/find_inhibitor_three_{model}.log","w")
		for ind in indexsss:
			indexss = [ind]
			# -------------------------------------------------------------------------------------------------
			df = pd.read_excel(f)
			...
			for indexs in indexss:
				...
				space = [
					...
					# Real(mins[selected_inhibitors[2]],maxs[selected_inhibitors[2]], name=selected_inhibitors[2]),
					Real(0.1,20, name=selected_inhibitors[0]),
					Real(0.1,20, name=selected_inhibitors[1]),
					Real(0.1,20, name=selected_inhibitors[2]),
					# Real(Pressures[0],Pressures[5], name='P (MPa)'),
				]
				func_vals_list = []
				temperature_obs, c_inhibitors, c_inhibitors1,c_inhibitors2 = [], [], [],[]
				for pressure in Pressures:
					...
					c_inhibitors2.append(result.x[2])
					result_temp = np.array([result.fun,pressure,result.x[0],result.x[1],result.x[2]])
				...
				df_func_vals.to_excel(f"./optimization/{save_path}/optimization_func_vals_{select_columns[indexs[0]]}_{select_columns[indexs[1]]}_{select_columns[indexs[2]]}.xlsx", index=False)
				print(temperature_obs,Pressures,c_inhibitors,c_inhibitors1,c_inhibitors2)
				ob = np.column_stack((temperature_obs,Pressures, c_inhibitors,c_inhibitors1,c_inhibitors2)) #.reshape(1,-1)
				df_results = pd.DataFrame(ob, columns=["Objective T (K)", "Objective P (MPa)",
													 f"{select_columns[indexs[0]]}", f"{select_columns[indexs[1]]}", f"{select_columns[indexs[2]]}"])
				# Save predicted temperature, pressure, and inhibitor concentrations
				df_results.to_excel(f"./optimization/{save_path}/optimization_results_{select_columns[indexs[0]]}_{select_columns[indexs[1]]}_{select_columns[indexs[2]]}.xlsx", index=False)
	...
	df_func_vals = pd.read_excel(f"./optimization/{save_path}/optimization_func_vals_{select_columns[indexss[0][0]]}_{select_columns[indexss[0][1]]}_{select_columns[indexss[0][2]]}.xlsx")
	print(df_func_vals)
	...
	df_results = pd.read_excel(f"./optimization/{save_path}/optimization_results_{select_columns[indexss[0][0]]}_{select_columns[indexss[0][1]]}_{select_columns[indexss[0][2]]}.xlsx")
	print(df_results)
