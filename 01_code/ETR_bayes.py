...
def objective(trial):
	...
	'min_samples_split': trial.suggest_int('min_samples_split', 2, 20, step=1),# Minimum samples required to split an internal node
	'min_samples_leaf': trial.suggest_int('min_samples_leaf', 1, 10, step=1), # Minimum samples required at a leaf node
	...
	for train_index, test_index in kf.split(X_train):
		...
		# Create the model
		model = ExtraTreesRegressor(**params,bootstrap=True)
...
def extract_metrics_from_trials(study,X,y,random_state=42):
	...
	for trial in study.trials:
		...
		for train_index, test_index in KFold(n_splits=cv, shuffle=True, random_state=random_state).split(X):
			...
			model = ExtraTreesRegressor(**param,bootstrap=True)
...
if __name__ == '__main__':
	...
	if train:
		...
		best_model = ExtraTreesRegressor(**best_params,bootstrap=True)
