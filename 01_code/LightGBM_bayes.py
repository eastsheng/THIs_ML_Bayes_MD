...
def objective(trial):
	...
	param = {
        'num_leaves': trial.suggest_int('num_leaves', 15, 256),  # Number of leaves
        'n_estimators': trial.suggest_int('n_estimators', 10, 1000, step=1),  # Number of base estimators
        'max_depth': trial.suggest_int('max_depth', 3, 50),  # Maximum tree depth
        'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.3,step=0.01,log=False),  # Learning rate
        'min_child_samples': trial.suggest_int('min_child_samples', 10, 100, step=1),
        'subsample': trial.suggest_float('subsample', 0.5, 1.0, step=0.05),  # Sample subsampling ratio
        'colsample_bytree': trial.suggest_float('colsample_bytree', 0.5, 1.0, step=0.05),  # Feature subsampling ratio
...
if __name__ == '__main__':
	...
	folder_path.mkdir(exist_ok=True)
	print_log(f'./save_model/{path}/{label}_best_{model}_model_{search}.log',"w")
	...
	# ay.scatter(y_other,resi_val,edgecolors=(0, 0, 0),label=r"$\regular Test_{\it new}$")
